...
capabilities:
  ...
  - "基于 assembly-order.yaml 的编排与变量替换系统：管理 Lysander / {{PRESIDENT_NAME}} / {{COMPANY_NAME}} 等模板变量的声明、默认值、替换时机；控制 fragment 拼装顺序(priority 字段)与条件包含(when 条件表达式)"
...
- 管理模板变量系统（Lysander 等），确保变量声明与替换的正确性
